...
Requires the ``prometheus`` extra::
    ...
    pip install redis-rate-limiter[prometheus]
"""
...
try:
    ...
    raise ImportError(
        ...
        "Install it with: pip install redis-rate-limiter[prometheus]"
    ) from exc
...
class PrometheusMetricsExporter:
    ...
    **Gauges** (updated on each consume event):
    ...
    - ``redis_rate_limiter_remaining_tokens{limiter_id}``
    - ``redis_rate_limiter_active_concurrency{limiter_id}``
    - ``redis_rate_limiter_buffer_depth{limiter_id}``

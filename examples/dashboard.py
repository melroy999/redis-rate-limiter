"""Reusable live dashboard for any rate limiter demo.

Renders concurrency, buffer, rate limit, and dispatcher status to the
terminal. Tracks per-window history automatically across render calls.
...
class Dashboard:
    """Live terminal dashboard driven by ``get_status()`` dicts."""
    ...
    def render(self, status: dict) -> None:
        """Clear the terminal and render the current status.

        Args:
            status: The dict returned by ``limiter.get_status()``.
        """
        elapsed = time.time() - self._start

        # Detect window rotation: val_current drops below the last seen value.

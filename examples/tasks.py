...
ASYNC_FUNC_PATH = "examples.tasks.async_mock_api_call"
ASYNC_FAILING_FUNC_PATH = "examples.tasks.async_mock_api_call_failing"
...
def mock_api_call_failing(user_id: int, priority: int = 100) -> None:
    ...
    delay = random.uniform(TASK_SLEEP_MIN, TASK_SLEEP_MAX)
    logger.debug(
        "user_id=%d prio=%d starting (will fail after %.3fs)", user_id, priority, delay
    )
    time.sleep(delay)
...
async def async_mock_api_call(user_id: int, priority: int = 100) -> None:
    """Async counterpart of ``mock_api_call`` using ``asyncio.sleep``."""
    delay = random.uniform(TASK_SLEEP_MIN, TASK_SLEEP_MAX)
    logger.debug(
        "user_id=%d prio=%d starting async (%.3fs delay)", user_id, priority, delay
    )
    await asyncio.sleep(delay)
    logger.debug("user_id=%d prio=%d done async", user_id, priority)


async def async_mock_api_call_failing(user_id: int, priority: int = 100) -> None:
    """Async counterpart of ``mock_api_call_failing`` that raises after a short delay."""
    delay = random.uniform(TASK_SLEEP_MIN, TASK_SLEEP_MAX)
    logger.debug(
        "user_id=%d prio=%d starting async (will fail after %.3fs)",
        user_id,
        priority,
        delay,
    )
    await asyncio.sleep(delay)
    logger.error("user_id=%d prio=%d raising RuntimeError async", user_id, priority)
    raise RuntimeError(f"Simulated async API failure for user_id={user_id}")

"""Unit tests for the ``BackendHealthMonitor`` background thread.

These tests verify the state-transition logic, thread lifecycle, and
opt-in detection of the ``BackendHealthMonitor``. The ``_run_once()``
method is tested directly (without starting the background thread) to
keep tests deterministic, following the same pattern used for
``DrainLoop`` testing via ``_drain_inner()``.

Fixture dependencies:
    - ``redis_client``, ``limiter_id``: from ``tests/conftest.py``.
    - ``generic_limiter``: from ``tests/implementations/conftest.py``.
"""

import logging
from unittest.mock import MagicMock

import pytest

from redis_rate_limiter.core.limiters import (
    BackendHealthMonitor,
    DistributedRateLimiterMixin,
)
from tests.helpers.utils import assert_log_emitted

# ---------------------------------------------------------------------------
# Helpers
# ---------------------------------------------------------------------------


@pytest.fixture
def mock_limiter():
    """Provide a mock limiter with an overridden ``_check_backend_health`` method."""
    limiter = MagicMock()
    limiter.id = "test_health_monitor"
    limiter._check_backend_health.return_value = True
    return limiter


@pytest.fixture
def monitor(mock_limiter):
    """Provide a ``BackendHealthMonitor`` instance backed by a mock limiter."""
    return BackendHealthMonitor(mock_limiter, interval=1.0)


# ---------------------------------------------------------------------------
# Behavioral tests
# ---------------------------------------------------------------------------


class TestBackendHealthMonitor:
    """Tests for ``BackendHealthMonitor`` state transitions and thread lifecycle."""

    @staticmethod
    def test_is_healthy_defaults_to_true(monitor):
        """Verify that a freshly created monitor reports healthy before any check runs."""
        # Assert
        assert monitor.is_healthy is True, (
            "monitor should default to healthy before first check"
        )

    @staticmethod
    def test_is_healthy_reflects_unhealthy_state(monitor, mock_limiter):
        """Verify that ``is_healthy`` returns ``False`` after the health check fails."""
        # Arrange
        mock_limiter._check_backend_health.return_value = False

        # Act
        monitor._run_once()

        # Assert
        assert monitor.is_healthy is False, (
            "monitor should reflect unhealthy state after failed check"
        )

    @staticmethod
    def test_is_healthy_recovers_after_healthy_check(monitor, mock_limiter):
        """Verify that ``is_healthy`` returns ``True`` after recovery from unhealthy state."""
        # Arrange
        mock_limiter._check_backend_health.return_value = False
        monitor._run_once()
        mock_limiter._check_backend_health.return_value = True

        # Act
        monitor._run_once()

        # Assert
        assert monitor.is_healthy is True, (
            "monitor should reflect healthy state after recovery"
        )

    @staticmethod
    def test_health_check_exception_transitions_to_unhealthy(monitor, mock_limiter):
        """Verify that an exception from the health check is treated as unhealthy."""
        # Arrange
        mock_limiter._check_backend_health.side_effect = ConnectionError("redis down")

        # Act
        monitor._run_once()

        # Assert
        assert monitor.is_healthy is False, "exception should be treated as unhealthy"

    @staticmethod
    def test_shutdown_stops_thread(monitor):
        """Verify that ``shutdown()`` terminates the background thread cleanly."""
        # Arrange
        monitor.start()

        # Act
        monitor.shutdown()

        # Assert
        assert monitor._thread is None or not monitor._thread.is_alive(), (
            "background thread should not be alive after shutdown"
        )

    @staticmethod
    def test_monitor_not_started_for_default_implementation(generic_limiter):
        """Verify that in-process backends (default ``_check_backend_health``) do not start a monitor."""
        # Assert
        assert generic_limiter._backend_health_monitor is None, (
            "in-process backends should not have a health monitor"
        )

    @staticmethod
    def test_monitor_not_started_when_drain_disabled(redis_client, limiter_id):
        """Verify that scheduler-only instances do not start a health monitor."""
        # Arrange
        from tests.implementations.conftest import MinimalRateLimiter

        limiter = MinimalRateLimiter(
            redis_client=redis_client,
            limiter_id=f"{limiter_id}_drain_disabled",
            limit=5,
            window=60,
            max_concurrency=2,
            drain_enabled=False,
        )

        # Assert
        try:
            assert limiter._backend_health_monitor is None, (
                "scheduler-only instances should not have a health monitor"
            )
        finally:
            limiter.shutdown()

    @staticmethod
    def test_monitor_uses_default_healthy_hook(generic_limiter):
        """Verify that the base mixin ``_check_backend_health`` returns ``True``."""
        # Arrange
        # Use a real limiter instance; MagicMock is incompatible with the
        # mutmut trampoline (object.__getattribute__ bypasses mock __getattr__).
        result = DistributedRateLimiterMixin._check_backend_health(generic_limiter)

        # Assert
        assert result is True, "base mixin health check should always return true"


# ---------------------------------------------------------------------------
# Observability tests
# ---------------------------------------------------------------------------


class TestBackendHealthMonitorObservability:
    """Observability tests for ``BackendHealthMonitor`` state-transition log emissions."""

    @staticmethod
    def test_unhealthy_transition_emits_warning_log(monitor, mock_limiter, caplog):
        """Verify that transitioning to unhealthy emits a WARNING log."""
        # Arrange
        mock_limiter._check_backend_health.return_value = False

        # Act
        with caplog.at_level(logging.WARNING, logger="redis_rate_limiter"):
            monitor._run_once()

        # Assert
        assert_log_emitted(
            caplog.records,
            level="WARNING",
            required_fragments=[f"limiter={monitor._limiter.id}"],
            message="should emit a warning log on transition to unhealthy",
        )

    @staticmethod
    def test_recovery_emits_info_log(monitor, mock_limiter, caplog):
        """Verify that recovery from unhealthy emits an INFO log."""
        # Arrange
        mock_limiter._check_backend_health.return_value = False
        monitor._run_once()
        mock_limiter._check_backend_health.return_value = True

        # Act
        with caplog.at_level(logging.INFO, logger="redis_rate_limiter"):
            monitor._run_once()

        # Assert
        assert_log_emitted(
            caplog.records,
            level="INFO",
            required_fragments=[f"limiter={monitor._limiter.id}"],
            message="should emit an info log on recovery to healthy",
        )

    @staticmethod
    def test_repeated_unhealthy_does_not_repeat_warning(monitor, mock_limiter, caplog):
        """Verify that consecutive unhealthy checks do not produce repeated warnings."""
        # Arrange
        mock_limiter._check_backend_health.return_value = False

        # Act
        with caplog.at_level(logging.WARNING, logger="redis_rate_limiter"):
            monitor._run_once()
            monitor._run_once()
            monitor._run_once()

        # Assert
        warning_records = [r for r in caplog.records if r.levelname == "WARNING"]
        assert len(warning_records) == 1, (
            "only one warning should be emitted for consecutive unhealthy states"
        )

    @staticmethod
    def test_exception_transition_emits_warning_log(monitor, mock_limiter, caplog):
        """Verify that a health check exception triggers the same warning as a ``False`` return."""
        # Arrange
        mock_limiter._check_backend_health.side_effect = RuntimeError("check failed")

        # Act
        with caplog.at_level(logging.WARNING, logger="redis_rate_limiter"):
            monitor._run_once()

        # Assert
        assert_log_emitted(
            caplog.records,
            level="WARNING",
            required_fragments=[f"limiter={monitor._limiter.id}"],
            message="should emit a warning log when health check raises an exception",
        )

    @staticmethod
    def test_healthy_to_healthy_emits_no_log(monitor, mock_limiter, caplog):
        """Verify that no log is emitted when the state remains healthy."""
        # Arrange
        mock_limiter._check_backend_health.return_value = True

        # Act
        with caplog.at_level(logging.DEBUG, logger="redis_rate_limiter"):
            monitor._run_once()

        # Assert
        transition_records = [
            r
            for r in caplog.records
            if r.levelname in ("WARNING", "INFO")
            and "health check" in r.getMessage().lower()
        ]
        assert len(transition_records) == 0, (
            "no transition log should be emitted when state remains healthy"
        )

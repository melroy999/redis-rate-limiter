...
This module contains only those tests that are specific to the RQ backend context.
...
class TestRQRateLimiterClassApi:
    ...
    def test_configure_without_queue_raises_error(redis_client):
        """Verify that ``configure`` raises an error when the ``queue`` argument is not provided."""
        # Arrange

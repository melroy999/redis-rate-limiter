"""Tests for the RQ task helper modules.

Fixture dependencies:
    - ``_reset_limiter_class_state``: from ``tests/implementations/rq/conftest.py``.
"""
...
from redis_rate_limiter.backends.rq.tasks.worker import generic_rate_limited_worker
...
class TestGenericWorkerTask:
    ...
    def test_generic_worker_resolves_and_executes_function():
        ...
        # Act
        # The @rate_limited decorator captures RQRateLimiter.get as a bound
        ...
        with patch.dict(RQRateLimiter._instances, {"worker_limiter": limiter}):
            with patch(
                "redis_rate_limiter.backends.rq.tasks.worker.import_string",
                return_value=target_func,
            ) as mock_import:
                result = generic_rate_limited_worker(
    ...
    def test_generic_worker_emits_debug_log(caplog):
        ...
        with patch.dict(RQRateLimiter._instances, {"worker_limiter": limiter}):
            with caplog.at_level(
                logging.DEBUG,
                logger="redis_rate_limiter.backends.rq.tasks.worker",
            ):
                with patch(
                    "redis_rate_limiter.backends.rq.tasks.worker.import_string",
                    return_value=target_func,

"""Tests for the Dramatiq task helper modules.
...
class TestGenericWorkerTask:
    ...
    def test_generic_worker_resolves_and_executes_function():
        ...
        # Act
        # The @rate_limited decorator captures DramatiqRateLimiter.get as a bound
        ...
        with patch.dict(DramatiqRateLimiter._instances, {"worker_limiter": limiter}):
            with patch(
                "redis_rate_limiter.backends.dramatiq.tasks.worker.import_string",
    ...
    def test_generic_worker_emits_debug_log(caplog):
        ...
        with patch.dict(DramatiqRateLimiter._instances, {"worker_limiter": limiter}):
            with caplog.at_level(
                logging.DEBUG,
                logger="redis_rate_limiter.backends.dramatiq.tasks.worker",
            ):
                with patch(
                    "redis_rate_limiter.backends.dramatiq.tasks.worker.import_string",
                    return_value=target_func,

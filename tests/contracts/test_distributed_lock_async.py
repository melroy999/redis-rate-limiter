"""Async contract tests for distributed lock implementations.

These tests define the expected behaviour for any async distributed lock
implementation. All async lock implementations are required to satisfy these
behavioural contracts.

This module mirrors ``test_distributed_lock.py`` with async method signatures.
"""
...
class AsyncDistributedLockContractTest:
    """Abstract test suite that any async distributed lock implementation must pass.
    ...
    Subclasses are required to provide the following:
        ...
        - create_lock: A fixture that returns a factory for async lock instances.
    ...
    @staticmethod
    async def test_lock_has_unique_token(
        async_redis_client, lock_key, create_lock
    ):

"""Async contract tests that any async RateLimiter implementation must satisfy.

These tests define the expected behaviour for all implementations of
``AbstractAsyncDistributedRateLimiter``. Any concrete async implementation
should inherit from ``AsyncRateLimiterContractTest`` and provide its own
async limiter fixture.

This module mirrors ``test_rate_limiter.py`` with async method signatures.
"""


class AsyncRateLimiterContractTest:
    """Abstract test suite that any async RateLimiter implementation must pass.

    Subclasses are required to provide the following:
        - limiter: An async fixture that returns a configured async limiter instance.
    ...
    @staticmethod
    async def test_get_inflight_key_format(limiter):
    ...
    @staticmethod
    async def test_limiter_has_required_attributes(limiter):

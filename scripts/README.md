...
All scripts are invoked automatically by `docker compose --profile mutate up --build --abort-on-container-exit --exit-code-from mutate`. The `mutate` service in `docker-compose.yml` runs:
...
Results are written to `./mutmut-results/` on the host via a bind mount.

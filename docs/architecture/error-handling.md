...
| 15 | `drain()` inner failure (any exception) | `drain()` | Any `Exception` | Increment failure counter, schedule recovery with backoff | `implementations/test_drain::test_drain_backoff_increases_with_consecutive_failures`, `test_drain_consume_exception_emits_error_log` | No |
...
| 32 | Sync function dispatched to async backend | `_dispatch_task()` (AsyncIO) | `TypeError` | Caught by `_run_task()` exception handler; task set cleaned up | `implementations/asyncio/test_asyncio_limiter::test_dispatch_sync_function_raises_type_error_with_message` | No |
...
| 38 | BLPOP timeout during `acquire()` | `acquire()` | `AcquireTimeout` | Propagates to caller; stale concurrency lease (if any) reclaimed by self-healing `ZREMRANGEBYSCORE` on next consume | `implementations/test_acquire::TestSyncAcquireBehavior::test_acquire_raises_acquire_timeout_on_blpop_none`, `implementations/test_acquire::TestAsyncAcquireBehavior::test_acquire_raises_acquire_timeout_on_blpop_none` | No |
...
| 40 | Acquire marker rejected by buffer (inflight collision) | `acquire()` | `RuntimeError` | Propagates to caller; no concurrency slot reserved | `implementations/test_acquire::TestSyncAcquireBehavior::test_acquire_raises_runtime_error_when_scheduling_fails`, `implementations/test_acquire::TestAsyncAcquireBehavior::test_acquire_raises_runtime_error_when_scheduling_fails` | No |
| 41 | `acquire()` called with `drain_enabled=False` | `acquire()` | `RuntimeError` | Propagates to caller | `implementations/test_acquire::TestSyncAcquireBehavior::test_acquire_raises_runtime_error_without_drain_loop`, `implementations/test_acquire::TestAsyncAcquireBehavior::test_acquire_raises_runtime_error_without_drain_loop` | No |
